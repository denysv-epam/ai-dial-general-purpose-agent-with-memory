...
class LongTermMemoryStore:
    ...
    async def delete_memories(
        ...
        """Delete memories that match the query precisely."""
        collection = await self._load_memories(api_key)
        if not collection.memories:
        ...
        for idx, score in enumerate(similarities):
            content_lower = collection.memories[idx].data.content.lower()
            token_match = any(token in content_lower for token in query_tokens)
            if score >= similarity_threshold and token_match:
        ...
        collection.memories = [
            mem for idx, mem in enumerate(collection.memories) if idx not in delete_set
        ]
        await self._save_memories(api_key, collection)
